...
"use strict";
(() => {
    const W = window, D = document, A = (a) => typeof a, TYPE = (e) => Object.prototype.toString.call(e), STR = A(''), ARR = TYPE([]), isSTR = (v) => A(v) === STR, isARR = Array.isArray || (e => TYPE(e) === ARR), DOC = D.documentElement || D.body, // html or body
    { error } = console, invalid = (e, f, t) => error(`Invalid argument of (${e})${(f ? ` for ${f}` : '')}.${(t ? ` Expecting (${t})` : '')}`), failTo = (e) => error(`Fail to ${e}`), newRegex = (pattern, flags) => new RegExp(pattern, flags), updateClass = (element, del, add) => {
        ...
        DARK = '_dark', set = (new_theme) => {
            ...
            STORE.set(KEY, theme);
        }, change = () => { set(list[list.indexOf(theme) + 1] || ''); }, media = W.matchMedia('(prefers-color-scheme: dark)');
        var list = [DARK], theme = '';
